...
const session = {
    ...
    getOrCreate: function(req) {
        const now = Date.now();
        var sessionId = req.query.session || ( now % 65536 + Math.floor( Math.random() * 4294967296 ) ).toString(16);
        var oSession = session.getById(sessionId) || {_id: sessionId, timestamp: now};
